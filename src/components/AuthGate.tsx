'use client';
import { useSession, signIn } from 'next-auth/react';

export default function AuthGate({ children }: { children: React.ReactNode }) {
  const { data: session, status } = useSession();

  if (status === 'loading') {
    return (
      <div style={{
        maxWidth: 520, margin: '0 auto', minHeight: '100vh',
        display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center',
        background: 'var(--bg)', fontFamily: "'Courier New', Courier, monospace",
      }}>
        <div style={{ fontSize: 13, color: 'var(--text3)' }}>Loading...</div>
      </div>
    );
  }

  if (!session) {
    return (
      <div style={{
        maxWidth: 520, margin: '0 auto', minHeight: '100vh',
        display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center',
        background: 'var(--bg)', fontFamily: "'Courier New', Courier, monospace",
        padding: '0 24px',
      }}>
        <div style={{ textAlign: 'center', width: '100%', maxWidth: 360 }}>
          <div style={{ fontSize: 28, fontWeight: 700, color: 'var(--green)', marginBottom: 6, letterSpacing: 1 }}>
            Piano Theory
          </div>
          <div style={{ fontSize: 13, color: 'var(--text3)', marginBottom: 40, lineHeight: 1.6 }}>
            Learn modes, scales, and chords.<br />
            Your progress saves across all devices.
          </div>

          <button
            onClick={() => signIn('google')}
            style={{
              display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 10,
              width: '100%', padding: '13px 20px', borderRadius: 10, fontSize: 14,
              background: 'var(--surface)', border: '1px solid var(--border)',
              color: 'var(--text)', cursor: 'pointer', fontFamily: 'inherit', fontWeight: 600,
            }}
          >
            <svg width="18" height="18" viewBox="0 0 18 18" fill="none">
              <path d="M17.64 9.205c0-.639-.057-1.252-.164-1.841H9v3.481h4.844a4.14 4.14 0 01-1.796 2.716v2.259h2.908c1.702-1.567 2.684-3.875 2.684-6.615z" fill="#4285F4"/>
              <path d="M9 18c2.43 0 4.467-.806 5.956-2.18l-2.908-2.259c-.806.54-1.837.86-3.048.86-2.344 0-4.328-1.584-5.036-3.711H.957v2.332A8.997 8.997 0 009 18z" fill="#34A853"/>
              <path d="M3.964 10.71A5.41 5.41 0 013.682 9c0-.593.102-1.17.282-1.71V4.958H.957A8.996 8.996 0 000 9c0 1.452.348 2.827.957 4.042l3.007-2.332z" fill="#FBBC05"/>
              <path d="M9 3.58c1.321 0 2.508.454 3.44 1.345l2.582-2.58C13.463.891 11.426 0 9 0A8.997 8.997 0 00.957 4.958L3.964 7.29C4.672 5.163 6.656 3.58 9 3.58z" fill="#EA4335"/>
            </svg>
            Continue with Google
          </button>

          <div style={{ marginTop: 12, fontSize: 12, color: 'var(--text3)' }}>
            No password needed — just your Google account.
          </div>

          <div style={{
            marginTop: 28, padding: '12px 14px', borderRadius: 8,
            background: 'var(--surface)', border: '1px solid var(--border)',
            fontSize: 12, color: 'var(--text2)', lineHeight: 1.7, textAlign: 'left',
          }}>
            <div style={{ fontWeight: 600, color: 'var(--text)', marginBottom: 6 }}>What gets saved to your account:</div>
            <div>Lesson completions</div>
            <div>Quiz scores and weak-topic tracking</div>
            <div>Works across all your devices</div>
          </div>
        </div>
      </div>
    );
  }

  return <>{children}</>;
}
